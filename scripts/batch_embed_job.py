...
Similar design to scripts/batch_entity_job.py (entity extraction's AWS
Batch job) in that this container never gets real object-storage
credentials -- the Go worker hands it short-lived presigned URLs instead.
It diverges on one point: the RESULT goes back via a presigned PUT to
object storage, not printed to stdout for CloudWatch Logs pickup the way
batch_entity_job.py's result is. That was the first design tried here
too, and it broke in production: a real document's embedding response
(hundreds of texts x 384 floats, full JSON float precision) runs a few
MB, but CloudWatch Logs caps a single log event at ~256KB and AWS's log
driver silently splits an oversized print() line across multiple events
with no marker that they were ever one line -- the Go side's log
reassembly (built for the normal one-event-per-line case) then produces
corrupted, unparseable JSON. Entity extraction's response is small enough
this never affected it. Embeddings' isn't, so the result needed a
transport with no practical size ceiling.
