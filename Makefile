...
# belong in the unit gate. reembed only touches memory/mock repositories in
# its tests, so it belongs here too despite existing to drive a real
# Postgres backfill in production.
UNIT_TESTPKG := ./internal/auth ./internal/account ./internal/config ./internal/server ./internal/graphedge ./internal/manifest ./internal/manifest/layout ./internal/llm ./internal/llm/inference \
